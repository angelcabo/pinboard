...
document.addEventListener('DOMContentLoaded', function () {
	...
	document.getElementById('save-tab-set').addEventListener('click', function () {
		save_tabs()
	});

	document.getElementById('unread-bookmarks').addEventListener('click', function () {
		chrome.tabs.create({url:'https://pinboard.in/toread/'});
		window.close();
	});

	document.getElementById('all-bookmarks').addEventListener('click', function () {
		chrome.tabs.create({url:'https://pinboard.in/'});
		window.close();
	});

	document.getElementById('network').addEventListener('click', function () {
		chrome.tabs.create({url: 'https://pinboard.in/network/'});
		window.close();
	});

	document.getElementById('tab-sets').addEventListener('click', function () {
		chrome.tabs.create({url: 'https://pinboard.in/tabs/'});
		window.close();
	});
});
